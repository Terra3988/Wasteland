...
public class Input {
    ...
    private final long[] frames = new long[1032];

    // Keyboard
    private final boolean[] keys = new boolean[1032];
    ...
    private void mouseButtonCallback(long window, int button, int action, int mode) {
        if (action == GLFW_PRESS) {
            keys[1024+button] = true;
            frames[1024+button] = _current;
        }
        else if (action == GLFW_RELEASE) {
            keys[1024+button] = false;
            frames[1024+button] = _current;
        }
    }

    private void keyCallback(long window, int key, int scancode, int action, int mode) {
        if (action == GLFW_PRESS) {
            keys[key] = true;
            frames[key] = _current;
        }
    ...
    public void dispose(long window) {
    ...
    public boolean isJustPressed(int key) {
        if (key < 0 || key >= 1024)
            return false;
        return keys[key] && (frames[key] == _current);
    }
    ...
    public boolean isJustClicked(int button) {
        int index = 1024 + button;
        return keys[index] && frames[index] == _current;
    }
